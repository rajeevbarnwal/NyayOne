...
def test_historical_populated_refusal_is_measured_before_current_head_upgrade():
    source = getsource(gate._run_populated_migration_probe)
    assert source.index('refused = _run_alembic') < source.index('application_upgrade =')
    assert 'historical_revision_unchanged' in source
    assert 'historical_schema_unchanged' in source
    assert 'historical_rows_unchanged' in source


def test_historical_lifecycle_finishes_before_current_forward_only_probe():
    source = getsource(gate._run_migration_lifecycle_probe)
    assert source.index('multi_step_downgrade_reupgrade =') < source.index('application_upgrade =')
    assert '_empty_application_roundtrip(scratch_url)' in source
    assert 'historical_head_revision = _current_revision(engine)' in source
