...
const [input,output,head,runId,artifactId,expectedArchiveSha256,archivePath]=process.argv.slice(2);
if(!input||!output||!archivePath||!/^\d+$/.test(runId)||!/^\d+$/.test(artifactId)||!/^[a-f0-9]{64}$/.test(expectedArchiveSha256))throw Error('USAGE: input new-output source-head run-id artifact-id expected-github-archive-sha256 downloaded-zip-path');
if(!(await lstat(archivePath)).isFile())throw Error('R2_NONREGULAR_ARCHIVE');
const archiveSha256=digest(await readFile(archivePath));
if(archiveSha256!==expectedArchiveSha256)throw Error('R2_ARCHIVE_DIGEST_MISMATCH');
