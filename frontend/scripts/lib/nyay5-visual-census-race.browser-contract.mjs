...
function markup(screenId = 'S-07', { wrongFont = false, heading = true, badIcon = false, lockup } = {}) {
  const revision = ['S-03', 'S-04', 'S-05', 'S-07', 'S-08', 'S-09', 'S-10', 'S-11', 'S-12', 'S-13', 'S-14'].includes(screenId);
...
describe('NYAY-5 real Chromium atomic visual census', () => {
  ...
  it('preserves the exact eleven-screen Revision L census and the legacy Carlito stack', async () => {
    await withPage(async (page) => {
      const rows = new Map();
      const actual = compileCensus(waitForVisualCensusSettled, rows);
      for (const screenId of ['S-03', 'S-04', 'S-05', 'S-07', 'S-08', 'S-09', 'S-10', 'S-11', 'S-12', 'S-13', 'S-14', 'S-15']) {
  ...
  it('does not allow a legacy screen to acquire the Revision L-only lockup exception', async () => {
    await withPage(async (page) => {
      await page.setContent(`<html data-theme="light"><body>${markup('S-15', { lockup: true })}</body></html>`);
      const rows = new Map();
      await compileCensus(waitForVisualCensusSettled, rows)(page, 'S-15');
      assertFields(rows.get('S-15'), { typographyExact: true, iconsExact: false });
    });
  });
  ...
  for (const [name, screenId, decoration, expected] of [
    ...
    ['legacy screen', 'S-15', '<span class="v321-profile-done__check" aria-hidden="true"><svg width="42" height="42"/></span>', false],
