...
export function AppShell({
  ...
  // S-14 owns its Revision L dashboard shell; module routes retain their shells.
  if (location.pathname === '/s-14') {
    return <main className="ls-v34-content" id="main-content">{children}</main>;
  }
  // S-15 owns its Revision L verification shell; verification remains server-owned.
  if (location.pathname === '/s-15') {
    return <main className="ls-v34-content" id="main-content">{children}</main>;
  }
  // S-16 owns its Revision L guardian shell; access remains server-owned.
